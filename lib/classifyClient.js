...
export async function requestClassifyPhoto(imageUrl, codeSet, options = {}) {
  const res = await fetch('/api/classify-photo', {
    ...
    body: JSON.stringify({
      imageUrl,
      codeSet,
      regionCrop: Boolean(options.regionCrop),
      productName: options.productName ? String(options.productName).trim() : undefined,
    }),
  });
