...
export async function createDocumentRow({
  ...
  const filePath = buildStorageObjectPath(cid, inserted.id, version, fileName);
  const { data, error } = await supabase
    .from('document')
...
export async function ensureDocumentStoragePath(documentId, companyId) {
  const cid = companyId || (await scopedCompanyId());
  const doc = await getDocument(documentId, cid);
  if (!doc) throw new Error('문서를 찾을 수 없습니다.');

  const filePath = buildStorageObjectPath(cid, doc.id, doc.version, doc.file_name);
  if (doc.file_path === filePath) return doc;
