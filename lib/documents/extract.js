...
export async function extractDocumentPages(buffer, fileType, opts = {}) {
  ...
  if (fileType === 'xlsx' || fileType === 'xls') {
    const { extractXlsxDocument } = await import('./xlsxExtract.js');
    const result = await extractXlsxDocument(buffer, opts);
    return {
      pages: result.pages,
      totalPages: result.totalPages,
      sheetIssues: result.sheetIssues || [],
      extractMethod: result.extractMethod || result.parseMode || 'table',
