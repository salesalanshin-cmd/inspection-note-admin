...
/**
 * @param {Buffer} buffer
 * @param {{ sheetNames?: string[]|null, parseMode?: 'table'|'flow' }} [opts]
 */
export async function extractXlsxDocument(buffer, opts = {}) {
  const logs = [];
  const parseMode = opts.parseMode === 'flow' ? 'flow' : 'table';
  ...
  for (let sheetIdx = 0; sheetIdx < targetNames.length; sheetIdx += 1) {
    ...
    if (parseMode === 'flow') {
      ...
      for (const chunk of chunks) {
        syntheticPageNo += 1;
        pages.push({ ...chunk, pageNo: syntheticPageNo });
        sheetChunkCount += 1;
      }
      sheetsSummary.push({
        sheetName,
    ...
    // 헤더명은 filled(병합 반영)에서 읽되 연속 중복·빈 칸 제거
...
/** 동기 미리보기용 (table 전용 — flow는 헤더 미리보기 의미 없음) */
export function previewXlsxWorkbook(buffer, opts = {}) {
  const parseMode = opts.parseMode === 'flow' ? 'flow' : 'table';
  if (parseMode === 'flow') {
    return { sheetNames: [], sheets: [], logs: [], parseMode: 'flow' };
  }
  const workbook = readWorkbook(buffer);
  const logs = [];
  const sheets = workbook.SheetNames.map((name) => {
    ...
    for (
      let i = dataStart;
      i < rawMatrix.length && previewRows.length < 10;
      i += 1
    ) {
      if (isEmptyRow(rawMatrix[i])) continue;
      previewRows.push({
        excelRow: originRow + i + 1,
        values: plan.indices.map((ci) =>
          compactSpaces(filledMatrix[i]?.[ci] || '')
        ),
      });
    }
    return {
  ...
  return { sheetNames: workbook.SheetNames, sheets, logs, parseMode: 'table' };
}
