...
async function processExtractStepInner(documentId, companyId) {
  ...
  } else if (isExcelFileType(fileType)) {
    const xlsxOpts = work.xlsxOptions || { useRecommended: true };
    const {
      pages,
      totalPages,
      sheetIssues,
      extractMethod,
    } = await extractDocumentPages(buffer, fileType, xlsxOpts);
    work.pages = pages;
    work.totalPages = totalPages;
    work.nextPage = totalPages + 1;
    work.extractMethod = extractMethod || 'table';
    work.sheetIssues = sheetIssues || [];
    await uploadWorkJson(companyId, documentId, doc.version, work);
  } else {
    const { pages, totalPages } = await extractDocumentPages(buffer, fileType);
    work.pages = pages;
...
async function finalizeExtraction(documentId, companyId, doc, work) {
  ...
  // 엑셀 시트 이미지 이슈 (page_no = 시트 순번)
  for (const sheetIssue of work.sheetIssues || []) {
    issueRows.push({
      document_id: documentId,
      company_id: companyId,
      page_no: sheetIssue.page_no,
      issue: sheetIssue.issue || 'image_not_parsed',
      resolved: false,
    });
  }
