 ...
 * 페이지 배열 → 조각
 *
 * ★ page_from === page_to (PDF/DOCX)
 * ★ 엑셀은 이미 조각화된 페이지(isExcelChunk) — page_from/to null, sectionLabel 유지
...
export function chunkPages(pages, extractMethod = 'text') {
  ...
  // 엑셀: extract에서 이미 800토큰 단위로 나눔 → 재분할하지 않음
  if (isExcelExtractMethod(extractMethod) || pages.some((p) => p.isExcelChunk)) {
    for (const page of pages) {
      ...
      chunks.push({
        chunk_index: 0,
        content: pageText,
        content_tokens: estimateTokens(pageText),
        page_from: page.pageFrom ?? null,
        page_to: page.pageTo ?? null,
        section_label: page.sectionLabel ?? null,
        extract_method: page.parseMode || extractMethod,
      });
    }
...
function isExcelExtractMethod(method) {
  return ['table', 'flow', 'mixed', 'xlsx', 'xls'].includes(String(method || ''));
}
