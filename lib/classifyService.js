 ...
 */
export async function classifyPhoto(imageUrl, codeSet) {
  ...
  const response = await client.messages.create({
    ...
    max_tokens: 256,
    system: buildClassifyPrompt(codeSet),
    messages: [
      {
        role: 'user',
        content: [
          ...
          {
            type: 'text',
            text: '이 사진을 분석하여 가장 적합한 오류 코드를 JSON 형식으로만 응답하세요.',
          },
        ],
      },
    ],
  ...
  for (let i = 0; i < items.length; i += BATCH_CHUNK_SIZE) {
    ...
    const settled = await Promise.allSettled(
      chunk.map(async (item) => {
        const data = await classifyPhoto(item.imageUrl, codeSet);
