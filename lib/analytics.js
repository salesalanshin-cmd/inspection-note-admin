...
/** 근무일 범위(전일 20시~익일 08시) 내 기록 필터 — 야간 종품(04~08시)까지 포함 */
 ...
 */
export function tagInspectionStage(record, workerDirectory) {
...
/**
 * 특정 작업자의 기록을 날짜(YYYY-MM-DD)별로 그룹핑합니다.
...
export function groupWorkerRecordsByDate(records, workerName) {
  ...
  for (const record of records || []) {
    ...
    const date = formatISODate(record.created_at);
    if (!buckets.has(date)) {
...
function filterRecordsInWorkRange(records, workDate) {
  ...
  rangeStart.setDate(rangeStart.getDate() - 1);
  rangeStart.setHours(20, 0, 0, 0);
  const rangeEnd = new Date(workDate);
...
export function buildDailyPerformance(
  ...
  const results = baseWorkers.map((row) => {
    ...
    if (!row.handles_frequent_check) {
      ...
      const missing = [];
      if (!c || c.noData) {
        // 기록 자체가 없거나 근무조 미정 → 전 단계 미실시로 간주
