...
function collectWorkerNamesInWorkRange(
  ...
  for (const r of docs || []) {
    if (!r.worker_name || !r.created_at) continue;
    const handlesDocs = directoryByName.get(r.worker_name)?.handles_documents ?? false;
...
export function buildDailyPerformance(
  ...
  // removed/excluded 필터 후, directory 값(없으면 기본값)으로 담당 업무·연락처 구성
...
const DEFAULT_CLOCK_IN_MINUTES = 8 * 60;
const DEFAULT_CLOCK_OUT_MINUTES = 17 * 60;
...
/** 근무일 기준 시각(자정 분) → Date. 08:00 미만은 익일 새벽으로 해석 */
function applyClockMinutesOnWorkDate(workDate, minutes) {
  const d = startOfDay(workDate);
...
/**
 * 개인별 3정5S 출퇴근 패턴 학습 (근무일 첫/마지막 기록의 중앙값)
 * @returns {{ clockInMinutes: number, clockOutMinutes: number, isPersonalized: boolean, sampleSize: number }}
 */
export function getWorkerClockPattern(
  ...
  for (let i = 0; i < days; i += 1) {
    ...
    workDate.setDate(workDate.getDate() - i);
    const dayRecords = (fivesReports || [])
      .filter((r) => r.worker_name === workerName && r.created_at)
      .filter((r) => {
        const { start, end } = getWorkRangeBounds(workDate);
        const t = new Date(r.created_at);
        return t >= start && t < end;
  ...
  if (sampleSize < 5) {
    return {
      clockInMinutes: DEFAULT_CLOCK_IN_MINUTES,
      clockOutMinutes: DEFAULT_CLOCK_OUT_MINUTES,
  ...
  return {
    clockInMinutes: medianMinutes(clockIns) ?? DEFAULT_CLOCK_IN_MINUTES,
    clockOutMinutes: medianMinutes(clockOuts) ?? DEFAULT_CLOCK_OUT_MINUTES,
    isPersonalized: true,
    sampleSize,
  };
}
 ...
 * @returns {Array<{
 *   worker_name: string,
 *   clockIn: { status: 'done'|'pending'|'missed', at: string|null },
 *   clockOut: { status: 'done'|'pending'|'missed', at: string|null },
...
export function buildClockInOutStatus(
  ...
  const excluded = getExcludedWorkerNames(workerDirectory);
  const removed = getRemovedWorkerNames(workerDirectory);
  const tolMs = CLOCK_TOLERANCE_MINUTES * 60 * 1000;

  // 당일 fives 기록이 있는 사람만 (결근자 제외). excluded/removed는 그 위 추가 필터.
  ...
  for (const worker_name of workers) {
    const pattern = getWorkerClockPattern(fivesReports, worker_name, workDate, 30);
    ...
    const dayRecords = (fivesReports || [])
      .filter((r) => r.worker_name === worker_name && r.created_at)
      .filter((r) => {
        const t = new Date(r.created_at);
    ...
    if (dayRecords.length >= 2) {
      ...
      if (nearOrAfterOut && !(clockInRecord && r.id === clockInRecord.id && t < clockOutAt.getTime() - tolMs)) {
        clockOutRecord = r;
      }
    }

    // 출근 창 밖이어도, 퇴근으로 잡힌 기록보다 이른 별도 기록이 없고
    // 출근 후보가 없을 때는 위 로직만 사용
...
export function buildClockInOutWeekSummary(
  ...
  for (let i = 0; i < days; i += 1) {
    ...
    const evalNow = new Date(rangeEnd.getTime() - 1);
