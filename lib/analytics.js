...
/** 특정 작업자의 근무일 범위(전일 20시~당일 24시) 내 기록만 반환 */
function recordsForWorkerInWorkRange(records, worker_name, workDate) {
  const rangeStart = new Date(workDate);
  rangeStart.setDate(rangeStart.getDate() - 1);
  rangeStart.setHours(20, 0, 0, 0);
  const rangeEnd = endOfDay(workDate);
  return (records || []).filter((r) => {
    if (r.worker_name !== worker_name || !r.created_at) return false;
    const t = new Date(r.created_at);
    return t >= rangeStart && t <= rangeEnd;
  });
}
