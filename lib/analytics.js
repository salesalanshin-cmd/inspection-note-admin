...
const NIGHT_SHIFT_START_MINUTES = 20 * 60; // 20:00
...
/**
 * 자정 기준 분(timeOfDayMinutes)이 주간 시작(08:00) / 야간 시작(20:00) 중
 * 원형(24h) 거리상 더 가까운 쪽으로 근무조를 판정합니다.
 * 거리가 같으면 주간(day)을 반환합니다.
 */
export function getNearestShiftStart(timeOfDayMinutes) {
  const m =
    ((Math.round(Number(timeOfDayMinutes)) % MINUTES_PER_DAY) + MINUTES_PER_DAY) %
    MINUTES_PER_DAY;
  const distDay = circularMinutesDistance(m, DAY_SHIFT_START_MINUTES);
  const distNight = circularMinutesDistance(m, NIGHT_SHIFT_START_MINUTES);
  return distDay <= distNight ? 'day' : 'night';
}

function shiftFromRecordTimestamp(createdAt) {
  const t = new Date(createdAt);
  if (Number.isNaN(t.getTime())) return null;
  return getNearestShiftStart(t.getHours() * 60 + t.getMinutes());
}
...
export function tagInspectionStage(record, workerDirectory, allWorkerRecords) {
  ...
  const inferredShift =
    manual || getNearestShiftStart(createdAt.getHours() * 60 + createdAt.getMinutes());
