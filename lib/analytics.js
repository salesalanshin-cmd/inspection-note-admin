...
/**
 * 오늘 근무일 기준 실시간 양호/미흡.
 * buildDailyPerformance를 기반으로 하되, "아직 시간이 안 지난" 미완료는
 * 미흡으로 보지 않고, 시간대가 지난 미실시만 warning으로 집계합니다.
 * NotifyReviewModal 호환 필드 + issueLabels를 포함합니다.
 *
 * @returns {Array<{
 *   worker_name: string,
 *   phone_number: string,
...
export function buildTodayRealtimePerformance(
  ...
  for (const row of performance) {
    ...
    if (row.frequentCheck.status !== 'na') {
      for (const stage of SHIFT_STAGES) {
        const done = complianceRow && !complianceRow.noData && complianceRow[stage]?.done;
        if (done) continue;
    ...
    const { end: shiftEnd } = getShiftStageWindow(workDate, shiftForWindow, '종품');
    const shiftEnded = now > shiftEnd;

    let fivesStatus = row.fives;
    if (row.fives.status === 'fail' && !shiftEnded) {
      fivesStatus = { status: 'ok', detail: '아직 진행 중' };
    }

    let documentsStatus = row.documents;
  ...
  const workers = (workerDirectory || [])
    .filter(
      (row) =>
        row.worker_name &&
        !row.excluded &&
        !excluded.has(row.worker_name) &&
        !removed.has(row.worker_name)
    )
    .map((row) => row.worker_name)
    .sort((a, b) => a.localeCompare(b, 'ko'));
