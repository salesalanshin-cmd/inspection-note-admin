...
export async function queryFivesReports({
  ...
  let query = supabase
    .from('fives_reports')
    .select(
      'worker_name, area_type, sos_error_code, sos_code, description, created_at, file_name'
    )
    .or(NOT_DELETED)
    .order('created_at', { ascending: false })
    .limit(MAX_ROWS);
  ...
  if (sosErrorCode) {
    query = query.or(`sos_error_code.eq.${sosErrorCode},sos_code.eq.${sosErrorCode}`);
  }
  query = applyDateRange(query, dateFrom, dateTo);
...
- fives_reports: 3정5S 기록 (worker_name, area_type, sos_error_code/sos_code, description, created_at, is_deleted)
