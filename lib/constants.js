...
/** worker_directory.process 값 (주조/사상/가공/검사/기타). null은 미지정 */
export const WORKER_PROCESSES = ['주조', '사상', '가공', '검사', '기타'];

/** worker_directory.role — 모바일 앱 화면 분기용 (권한 아님) */
export const WORKER_ROLES = [
  { value: 'inspector', label: '검사자' },
  { value: 'worker', label: '작업자' },
  { value: 'manager', label: '관리자' },
];

/** worker_directory.lang — 앱 UI·콘텐츠 번역 언어 (nationality와 별개) */
...
/** 국적 → 권장 lang (제안 문구용, 자동 변경하지 않음) */
export const NATIONALITY_LANG_HINTS = {
  베트남: 'vi',
  캄보디아: 'km',
  태국: 'th',
};

const WORKER_ROLE_VALUES = new Set(WORKER_ROLES.map((r) => r.value));
const WORKER_LANG_VALUES = new Set(WORKER_LANGS.map((l) => l.value));

export function normalizeWorkerRole(role) {
  return WORKER_ROLE_VALUES.has(role) ? role : 'inspector';
}

export function normalizeWorkerLang(lang) {
  return WORKER_LANG_VALUES.has(lang) ? lang : 'ko';
}

export function workerRoleLabel(role) {
  return WORKER_ROLES.find((r) => r.value === role)?.label ?? '검사자';
}

export function workerLangLabel(lang) {
  return WORKER_LANGS.find((l) => l.value === lang)?.label ?? '한국어';
}

/** 현황 화면 공정 필터 기본값 — 처음 열면 주조 담당자만 표시 */
export const DEFAULT_PROCESS_FILTER = '주조';

/** 공정 필터 "전체" — 미지정(process null) 작업자 포함 */
...
// 알려진 불량 코드 -> 한글 라벨 (DEFECT_CODE_DEFINITIONS 기반, 없는 코드는 defect_type 사용)
export const DEFECT_CODE_LABELS = Object.fromEntries(
  DEFECT_CODE_DEFINITIONS.map((d) => [d.code, d.label])
);
