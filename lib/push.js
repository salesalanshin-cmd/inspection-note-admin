...
/**
 * 최근 1분 내 동일 thread + type 발송 기록이 있으면 중복으로 본다.
...
async function findRecentPushEvent({
  ...
  if (!companyId || !threadId || !pushType) return null;
  const since = new Date(Date.now() - withinMs).toISOString();
  try {
    const { data, error } = await supabase
      .from('event_log')
      .select('id, created_at, meta, event_type')
      .eq('company_id', companyId)
      .eq('thread_id', threadId)
      .in('event_type', ['push_sent', 'push_failed'])
    ...
    const match = (data || []).find((row) => row?.meta?.type === pushType);
    return match ? { id: match.id, created_at: match.created_at } : null;
  } catch (err) {
    // eslint-disable-next-line no-console
